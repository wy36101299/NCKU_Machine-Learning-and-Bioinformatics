...
def process_options(argv=sys.argv):
    ...
    usage = """\
Usage: gridregression.py [-log2c begin,end,step] [-log2g begin,end,step] [-log2p begin,end,step] [-v fold] 
...
#            if find(line,"absolute") != -1:
            if find(line,"correlation") != -1:
...
#                if mse < best_mse:
                if mse > best_mse:
                    ...
                    best_g = 2.0**g1
                    best_p = 2.0**p1
#                print " (best c=%s, g=%s, p=%s, mae=%s)" % \
                print " (best c=%s, g=%s, p=%s, cc=%s)" % \
